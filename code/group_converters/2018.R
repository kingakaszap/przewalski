...
# 2024-01-19
# wtf how did this take me so long
...
total_2018 <- data.frame()  # Initialize an empty data frame to store the total

# Get the Excel file name (assumed to be a year)
excel_file <- "2018.xlsx"
...
for (sheet_name in all_sheets) {
  ...
  total_2018 <- rbind(total_2018, sheet_data)
}

View(total_2018)

# total_2018$name <- gsub("\\?", "", total_2018$name)
# total_2018$harem <- gsub("\\([^)]+\\)", "", total_2018$harem)

unique(total_2018$date)

total_current_2018<- rbind(total_current_2017, total_2018)

View(total_current_2018)

write_xlsx(total_current_2018, "przewalski/data/groups/long/total_current_2018.xlsx")
